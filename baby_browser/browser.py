...
class browser:
    ...
    BROWSER_FOLDER = os.path.join(expanduser('~'),"browser") 
    ...
    def __init__(self):
        ...
        try:
            with open(browser.BOOKMARKS_FILE, 'rb') as bookmarks_file:
                self.bookmarks = pickle.load(bookmarks_file)
        except:
            with open(browser.DEFAULT_BOOKMARKS_FILE, 'rb') as bookmarks_file:
                self.bookmarks = pickle.load(bookmarks_file)
            if not os.path.isdir(browser.BROWSER_FOLDER):
                os.makedirs(browser.BROWSER_FOLDER) 
        with open(browser.DEFAULT_CSS, 'r') as default_css:
    ...
    def on_close(self):
        ...
        if self.bookmarks_has_changed:
            with open(browser.BOOKMARKS_FILE, 'wb') as bookmarks_file:
